...
const struct option config_options[] = {
    ...
    { "traffic-iface", required_argument, 0, 0 },
...
static void publish_status(const char *event_type, bool success, const char *message, bool verbose, json_object *extras) {
    ...
    if (mqtt_enabled) {
        ...
        if (strcmp(event_type, "heartbeat") == 0) {
            ...
            json_object_object_add(json, "traffic_targets", json_object_new_int(traffic_config.count));
        }

        if (extras) {
            json_object_object_foreach(extras, key, val) { json_object_object_add(json, key, json_object_get(val)); }
        }
...
static void traffic_poll_host(const char *host, const char *community, const int *target_indices, int target_count) {

    int item_count            = target_count * TRAFFIC_OID_COUNT;
    snmp_get_item_t *items    = calloc((size_t)item_count, sizeof(snmp_get_item_t));
    if (!items)
        return;

    for (int t = 0; t < target_count; t++) {
        const traffic_target_t *target = &traffic_config.targets[target_indices[t]];
        for (int o = 0; o < TRAFFIC_OID_COUNT; o++)
            snprintf(items[t * TRAFFIC_OID_COUNT + o].oid, SNMP_OID_MAX, "%s.%d", traffic_oid_bases[o], target->index);
    }

    char err[256], status_message[512];
    if (!snmp_get(host, community, items, item_count, err, sizeof(err))) {
        snprintf(status_message, sizeof(status_message), "snmp get %s failed: %s", host, err);
        publish_status("traffic", false, status_message, traffic_config.verbose, NULL);
        free(items);
        return;
    }
    ...
    for (int t = 0; t < target_count; t++) {
        ...
        if (!r[0].value.valid || !r[1].value.valid) {
            snprintf(status_message, sizeof(status_message), "%s/%s no counter data", target->host, target->name);
            publish_status("traffic", false, status_message, traffic_config.verbose, NULL);
            continue;
        }
        ...
        snprintf(status_message, sizeof(status_message), "%s/%s %lds rx:%s(%.2fMbps) tx:%s(%.2fMbps)%s", target->host, target->name, (long)duration, in_str,
                 (double)in_bps / 1e6, out_str, (double)out_bps / 1e6, errors_str);

        json_object *extras = json_object_new_object();
        json_object_object_add(extras, "device", json_object_new_string(target->host));
        json_object_object_add(extras, "interface", json_object_new_string(target->name));
        json_object_object_add(extras, "duration", json_object_new_int64((int64_t)duration));
        json_object_object_add(extras, "in_octets", json_object_new_int64((int64_t)in_bytes));
        json_object_object_add(extras, "out_octets", json_object_new_int64((int64_t)out_bytes));
        json_object_object_add(extras, "in_packets", json_object_new_int64((int64_t)in_pkts));
        json_object_object_add(extras, "out_packets", json_object_new_int64((int64_t)out_pkts));
        json_object_object_add(extras, "in_errors", json_object_new_int64((int64_t)in_errs));
        json_object_object_add(extras, "out_errors", json_object_new_int64((int64_t)out_errs));
        json_object_object_add(extras, "in_bps", json_object_new_int64((int64_t)in_bps));
        json_object_object_add(extras, "out_bps", json_object_new_int64((int64_t)out_bps));
        json_object_object_add(extras, "oper_status", json_object_new_int((int32_t)oper_status));
        json_object_object_add(extras, "speed_mbps", json_object_new_int64((int64_t)speed_mbps));
        publish_status("traffic", true, status_message, traffic_config.verbose, extras);
...
static void traffic_poll_all(void) {
    ...
    for (int i = 0; i < traffic_config.count; i++) {
        if (processed[i])
            continue;
        int indices[MAX_TRAFFIC_TARGETS];
        int count        = 0;
        indices[count++] = i;
        processed[i]     = true;
        for (int j = i + 1; j < traffic_config.count; j++) {
            if (processed[j])
                continue;
            if (strcmp(traffic_config.targets[i].host, traffic_config.targets[j].host) == 0 && strcmp(traffic_config.targets[i].community, traffic_config.targets[j].community) == 0) {
                indices[count++] = j;
                processed[j]     = true;
            }
        }
        traffic_poll_host(traffic_config.targets[i].host, traffic_config.targets[i].community, indices, count);
    }
}
...
void process_loop(void) {
    ...
    while (running) {
        ...
        if (running && intervalable(timing_config.heartbeat_period, &last_heartbeat, false))
            publish_status("heartbeat", do_heartbeat(status_message, sizeof(status_message)), status_message, heartbeat_config.verbose, NULL);
...
bool config_load_traffic_targets(void) {
    ...
    traffic_config.count = 0;

    const char *value = config_get_string("traffic-iface", "");
    if (strlen(value) > 0)
        if (config_parse_traffic_iface(value, &traffic_config.targets[traffic_config.count]))
            traffic_config.count++;

    for (int i = 1; i <= MAX_TRAFFIC_TARGETS; i++) {
        char key[64];
        snprintf(key, sizeof(key), "traffic-iface[%d]", i);
        const char *idx_value = config_get_string(key, "");
        if (strlen(idx_value) == 0) {
            if (i > 1)
                break;
            continue;
        }
        if (traffic_config.count < MAX_TRAFFIC_TARGETS)
            if (config_parse_traffic_iface(idx_value, &traffic_config.targets[traffic_config.count]))
                traffic_config.count++;
    }
    return traffic_config.count > 0;
}
...
bool config_load_settings(const int argc, const char *argv[]) {
    ...
    traffic_config.enabled = timing_config.traffic_poll_period > 0 && (traffic_config.count > 0);
    traffic_config.verbose = config_get_bool("traffic-verbose", true);
    if (traffic_config.enabled)
        for (int i = 0; i < traffic_config.count; i++)
            printf("traffic: [%d] '%s/%s' index=%d (%d, %s)\n", i + 1, traffic_config.targets[i].host, traffic_config.targets[i].name, traffic_config.targets[i].index,
                   timing_config.traffic_poll_period, traffic_config.verbose ? "verbose" : "quiet");
...
int main(int argc, const char **argv) {
    ...
    snmp_begin();
    publish_status("startup", true, "daemon started", true, NULL);
    process_loop();
    publish_status("shutdown", true, "daemon stopped", true, NULL);
    snmp_end();
